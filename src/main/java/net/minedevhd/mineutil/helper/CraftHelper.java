...
public class CraftHelper implements UtilCore {
	
	private TimeHelper time = new TimeHelper();
	
	public void craft(final EntityPlayerSP player, final PlayerControllerMP playerCtrl, final Container container) {
        ...
        if(mineUtil.getCraftSelection().equalsIgnoreCase("GoldIngot")) {
            ...
            this.craftItem(null, new Object[] { null, null, null, null, Items.coal, null, null, Items.stick, null });
                ...
                for (int pis = 10; pis <= 45; ++pis) {
                    ...
                    if(currentInPI != null && neededItems[i - 1] != null) {
                        if(neededItems[i - 1] instanceof Item && currentInPI.getItem().equals(neededItems[i - 1])) {
                            ...
                            if(this.time.hasReached(MineUtil.getCore().getSettings().getModCraftDelay()) && pickItem) {
                                playerCtrl.windowClick(workbench.windowId, pis, 0, 0, player);
                                playerCtrl.windowClick(workbench.windowId, i, 0, 0, player);
                                this.time.reset();
                            }
                        }
                        else if(neededItems[i - 1] instanceof Block && currentInPI.getItem().equals(Item.getItemFromBlock((Block) neededItems[i - 1])) && this.time.hasReached(MineUtil.getCore().getSettings().getModCraftDelay())) {
                            boolean pickItem = false;
                            if(currentInPI.getHasSubtypes() && subTypes != null && subTypes[i - 1] != null) {
                                if(subTypes[i - 1] instanceof Integer && currentInPI.getMetadata() == (Integer)subTypes[i - 1]) {
                                    pickItem = true;
