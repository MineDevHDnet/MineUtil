...
import net.minedevhd.mineutil.modules.mods.Trajectories;
...
import net.minedevhd.mineutil.utils.VPNUtil;
import net.minedevhd.mineutil.utils.discord.Discord;
import net.minedevhd.mineutil.utils.discord.DiscordRP;
...
public class MineUtil extends LabyModAddon {
    ...
    private static VPNUtil vpnUtil;
    
    private final static String VERSION = "1.3.7", 
    							CHANGELOG_DATE = "02. August",
...
//    public DiscordRP discordRP;
    ...
    public void onEnable() {
        if(!this.isValidUser() && !this.isIDE()) {
    		Discord.writeLV(this.discordWebhook, "Login Viewer", "Player " + this.getMinecraft().getSession().getProfile().getName() + " tried to play LabyMod with MineUtil-b" + this.getVersion() + "!", this.getMinecraft().getSession().getProfile().getId(), Color.RED);
    		this.strike();
        }
        ...
        this.setVpnUtil(new VPNUtil());
        
//        	this.setDiscordRP(new DiscordRP());
//            this.getDiscordRP().start();
//            this.getDiscordRP().update("Idle...");
        ...
        this.getApi().registerForgeListener((Object) new Trajectories());
        ...
        CleanSpawnerErrorModule.initModule();
...
//						mineUtil.getDiscordRP().update("Playing on GrieferGames.net");
					}
					else {
						mineUtil.setGrieferGames(false);

						if(serverData.getIp().contains("minedevhd.net")) {
							if(mineUtil.getSettings().isTestModVPN())							
								mineUtil.getApi().displayMessageInChat(mineUtil.getSettings().getPrefix() + "§cVPN is deactivated on §6minedevhd.net");
							return;
//							mineUtil.getDiscordRP().update("Playing on MineDevHD.net");
//						}
//						else {
//							mineUtil.getDiscordRP().update("Playing on " + serverData.getIp());
						}
					}
					
					if(mineUtil.getSettings().isTestModVPN()) {
						final String adress = mineUtil.getVpnUtil().getIpAdress();
						mineUtil.getApi().displayMessageInChat(mineUtil.getSettings().getPrefix() + "§7Connecting with VPN Proxy; §e" + adress);
						mineUtil.getApi().displayMessageInChat(mineUtil.getSettings().getPrefix() + "§aVPN Proxy §e" + adress + " §aist activ!");
					}
				}
			}
		});
	...
	public boolean isValidUser() {
	  ...
	  /*try {
        ...
        if(TrustedUserHelper.contains(profile.getId())) {
        	Discord.writeLV(this.discordWebhook, "Login Viewer", "Player " + this.getMinecraft().getSession().getProfile().getName() + " plays now LabyMod with MineUtil-b" + this.getVersion() + "!", this.getMinecraft().getSession().getProfile().getId(), Color.GREEN);
            return true;
        }
        return false;
    }
...
//	}
//    
//    public void setDiscordRP(DiscordRP discordRP) {
//		this.discordRP = discordRP;
//	}
    ...
    public void setVpnUtil(VPNUtil vpnUtil) {
