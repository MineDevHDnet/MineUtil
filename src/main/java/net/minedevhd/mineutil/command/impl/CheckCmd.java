...
public class CheckCmd extends Command {
	...
	public void onCommand(String[] args, String command) {
		if(!MineUtil.getUtilCore().getMCCore().isSingleplayer()) {
			final PacketBuffer buffer = new PacketBuffer(Unpooled.buffer());
			buffer.writeString("Check");
			MineUtil.getUtilCore().getMCCore().thePlayer.sendQueue.addToSendQueue(new C17PacketCustomPayload("mc|AdvCdm", buffer));
		}
	}
